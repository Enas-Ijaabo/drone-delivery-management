...
# Cleanup: ORDER_ID is in picked_up state, assigned to DRONE1
req_auth POST /orders/$ORDER_ID/deliver "$DRONE1_TOKEN" '' 200 >/dev/null 2>&1 || \
  req_auth POST /orders/$ORDER_ID/fail "$DRONE1_TOKEN" '' 200 >/dev/null 2>&1
