...
# ORDER_ID is now in delivered state (terminal), drone is free
...
# ORDER2_ID is now in failed state (terminal), drone is free
...
# ORDER3_ID is now in failed state (terminal), drone is free
...
# Cleanup: fail ORDER6_ID to free drone
req_auth POST /orders/$ORDER6_ID/fail $DRONE1_TOKEN '' 200 >/dev/null
